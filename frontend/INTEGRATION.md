...
`POST /analysis` receives the original action list, not a browser-provided Score
or simulation result. The backend recomputes the scenario before calling the AI.
Its camelCase fields (`executiveSummary`, `keyImprovements`, `strategicRecommendations`,
