...
- Опциональный AI-анализ на русском языке: OpenAI получает уже рассчитанный результат и объясняет его, не пересчитывая показатели.
- Pure-функция backend для поиска до трёх лучших валидных альтернатив с заменой одной выбранной меры.
...
5. Frontend показывает результат, изменения по районам и рекомендации. Отдельно вызывается AI-анализ; при отсутствии ключа OpenAI или ошибке доступен детерминированный fallback-анализ.
...
Числа рассчитываются только в детерминированных движках backend. Маршрут `/analysis` сначала заново запускает симуляцию, а затем передаёт её результат в OpenAI. Модель не должна изменять или вычислять метрики и Score.
...
Для локального запуска `frontend/.env` можно оставить с настройками из примера: Vite по умолчанию проксирует `/simulate` и `/analysis` на `http://127.0.0.1:4000`.
...
4. В текущем наборе тестов для него зафиксированы: базовый Score `52.56`, итоговый Score `56.54`, изменение `+3.98`.
...
- Модуль подбора альтернатив реализован в backend как pure-функция, но отдельный HTTP-маршрут или элемент интерфейса для него пока не добавлен.
